...
- Chat coach: plain text on white; no `---`, dialog prefixes, or meta filler; no markdown tables (numbered lists for comparisons); client keeps raw `<options>` in message state, DB/store gets stripped text.
...
- Stack: Next.js App Router, Supabase (auth/data), Mistral for chat/canvas-worker; n8n on Hostinger (REST + instance-level MCP via `N8N_MCP_URL`/`N8N_MCP_TOKEN` in `lib/n8n-mcp-bridge.ts` — not `N8N_API_KEY`); deploy sets `availableInMCP: true` automatically.
